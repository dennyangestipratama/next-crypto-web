...
import { CryptoPriceInterface, CryptoDataListMobileProps } from '@interface/interface';
...
const CryptoDataListSection: React.FC<CryptoDataListMobileProps> = ({
  ...
  return (
    <div>
      ...
      <a href={`${endpoints.webMarket}/${currency.currencySymbol.toLowerCase()}`}>
        <div className="p-4 flex items-center border-b">
          <Logo url={currency.logo} color={currency.color} />
          <div className="pl-6 flex flex-1">
            <div className="flex-1">
              <span className="block font-semibold text-custom-black-01">{currency.name}</span>
              <span className="block text-sm text-custom-grey-02">{currency.currencySymbol}</span>
            </div>
          </div>
          <div>
            ...
            <div className="flex justify-end text-sm">{percentageIndicator(getLatestPrice)}</div>
          </div>
        </div>
      </a>
    </div>
